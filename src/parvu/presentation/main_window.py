...
from parvu.presentation.workers import QueryWorker, UniqueValuesWorker
...
class MainWindow(QMainWindow, ThemeableMixin):
    ...
    def _browse_file(self) -> None:
        ...
        if file_path:
            if self._window_service:
                self._window_service.create_window(Path(file_path))
            else:
    ...
    def _on_page_loaded(self, df) -> None:
        ...
        if self._engine:
            ...
            self.statusBar().showMessage(
                self._t("status.page_info", page=self._current_page, total_pages=self._engine.total_pages)
            )
